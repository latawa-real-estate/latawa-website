...
/* Contact form -> Google Sheet + auto-reply. Paste the Web App URL
   from your deployed Apps Script (see google-apps-script/Code.gs). */
const GOOGLE_SHEETS_ENDPOINT =
  "https://script.google.com/macros/s/AKfycbwmbriWgiRaO5Y4KANn_jaiVtpV4n97Z_UxsE5qh406jwjwKYlPBcsum7oPzJFYZXMN_A/exec";
...
async function injectComponents() {
  ...
  try {
    ...
    hw.childNodes.forEach((n) => {
      if (n.nodeType === 1 || n.nodeType === 3)
        par.insertBefore(n.cloneNode(true), ref);
    });
  } catch (e) {
...
async function applyBranding() {
  ...
  if (site.logo) {
    ...
    if (logoUrl) {
      document
        .querySelectorAll(".nav-logo .logo-svg, .footer-logo .logo-svg")
        .forEach((svg) => {
  ...
  if (site.siteName) {
    document
      .querySelectorAll(".logo-name")
      .forEach((el) => (el.textContent = site.siteName));
  }
  if (site.siteSubtitle) {
    document
      .querySelectorAll(".logo-sub")
      .forEach((el) => (el.textContent = site.siteSubtitle));
  }

  // Footer tagline + about
  if (site.tagline) {
    document
      .querySelectorAll(".footer-tagline, .footer-credit")
      .forEach((el) => (el.textContent = site.tagline));
  }
  if (site.footerAbout) {
  ...
  if (site.copyrightYear) {
    ...
    if (copy)
      copy.textContent = `© ${site.copyrightYear} Latawa Real Estate. All rights reserved.`;
  }
...
function initScrollReveals() {
  ...
  if (typeof gsap !== "undefined" && typeof ScrollTrigger !== "undefined") {
    ...
    const staggerGroups = [
      { sel: ".prop-card", stagger: 0.07 },
      { sel: ".service-card", stagger: 0.07 },
      { sel: ".trust-card", stagger: 0.07 },
      { sel: ".process-step", stagger: 0.1 },
      { sel: ".gallery-item", stagger: 0.06 },
      { sel: ".stat-card", stagger: 0.07 },
      { sel: ".founder-card", stagger: 0.08 },
      { sel: ".value-card", stagger: 0.07 },
      { sel: ".timeline-item", stagger: 0.06 },
      { sel: ".why-row", stagger: 0.07 },
      { sel: ".package-card", stagger: 0.08 },
      { sel: ".project-card", stagger: 0.07 },
    ];
...
function initContactForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (
      ...
      GOOGLE_SHEETS_ENDPOINT.includes("PASTE_YOUR")
    ) {
      console.warn(
        "[Contact Form] GOOGLE_SHEETS_ENDPOINT is not configured in loader.js yet.",
      );
    ...
    // Apps Script Web Apps don't return CORS headers on their response,
    // so the request is fired with mode:'no-cors' — it still reaches the
    // script and runs (row added, emails sent), we just can't read the
    // result back. A network-level failure (offline, wrong URL) is the
    // only thing that lands in .catch().
...
async function boot() {
  ...
  const pageInitPromise =
    typeof window.__pageInit === "function"
      ? Promise.resolve(window.__pageInit())
      : Promise.resolve();
